...
@Service
@Transactional
public class AttendanceForPlansAppServiceImpl implements AttendanceForPlansAppService {

	@Autowired
	private AttendanceForPlansAppDao attendanceForPlansDao;//考勤记录数据层
    
	@Autowired
	private UsersAppDao usesDao;//考勤记录数据层
	
    @Autowired
    private CoverWorkAppService coverWorkAppService;//代班记录业务层
    
    @Autowired
    private LeaveAppService leaveAppService;//请假记录业务层
    
    @Autowired
    private TaskInformationAppService taskInformationAppService;//任务记录业务层
    
    @Autowired
    private AdminLogsService adminLogsService;//系统日志业务层
    
    @Autowired
    private AttendanceForPointAppService attendanceForPointAppService;//考勤记录对应责任点记录业务层
    ...
    public List<Map<String, Object>> getAllCityWorkAttForApp(Integer employeeId) {
        ...
        AttendanceForPlansApp att = attendanceForPlansDao.getInfoById("getAllCityWorkAttLastTimeForApp", employeeId);
        // 获取人员角色
        UsersApp users = usesDao.getInfoById("getUserInfoByEmployeeIdApp", employeeId);
        // 查询条件
        Map<String, Object> selectMap = new HashMap<String, Object>();
        // 员工ID
        selectMap.put("employeeId", employeeId);
        // 角色ID
        selectMap.put("roleId", users.getRoleId());
        // 是
        if (att != null) {
            return attendanceForPlansDao.getInfoListMapByMap("getAllCityWorkAttLastForApp", selectMap);
        // 不是    
        } else {
            return attendanceForPlansDao.getInfoListMapByMap("getAllCityWorkAttForApp", selectMap);
        }
    ...
    public void autoGenAttendanceForPlans() {
        ...
        //2.自动生成考记录对应的责任点记录
        attendanceForPointAppService.batchAddAttendanceForPointApp();//自动生成考记录对应的责任点记录，2017-01-10
        ...
        for (CoverWorkApp coverWork : lstCoverWork) {
            ...
            for (AttendanceForPlansApp att : lstAttForPlans) {
                ...
                if (coverFromTime.compareTo(dutyOnTime) <= 0 && dutyOnTime.compareTo(coverToTime) <= 0) {
                    ...
                    createCoverWorkAtt(isOldPlans, coverId, dutyAreaId, dutyOnTime, dutyOffTime, roleId, att, coverWork.getLeaveId());
    ...
    public void createCoverWorkAtt(int isOldPlans, int coverId, int dutyAreaId, Date dutyOnTime, Date dutyOffTime, 
            int roleId, AttendanceForPlansApp attendance, int leaveId){
        ...
        if (isOldPlans == 2) {
            ...
            //删除掉代班人考勤记录对应的责任点记录
            AttendanceForPointApp attendanceForPointApp = new AttendanceForPointApp();
            attendanceForPointApp.setEmployeeId(coverId);
            attendanceForPointApp.setDutyOnTime(dutyOnTime);
            attendanceForPointApp.setDutyOffTime(dutyOffTime);
            attendanceForPointApp.setAreaId(dutyAreaId);
            attendanceForPointApp.setRecordDate(attendance.getRecordDate());
            attendanceForPointAppService.deleteAttendanceForPointAppByBeanApp(attendanceForPointApp);//根据bean删除代班人考勤记录对应的责任点记录。2017-01-10
        }
        ...
        if (roleId == 2) {
            ...
            attendanceForPointApp.setAreaId(dutyAreaId);
            attendanceForPointApp.setRecordDate(attendance.getRecordDate());
            attendanceForPointApp.setRoleId(roleId);
            attendanceForPointApp.setIsCoverwork(1);
            // 存在
            if (lstAttForPlans != 0) {
                // 是否代班人考勤代班记录(1是 2不是)
                ...
                //更新代班人考勤记录对应的责任点记录中的is_coverwork为1，表示是代班责任点
                attendanceForPointApp.setIsCoverwork(1);
                attendanceForPointAppService.updateAttendanceForPointAppByBeanApp(attendanceForPointApp);//根据bean更新代班人考勤记录对应的责任点记录。2017-01-10
                ...
                //新增考勤记录对应的代班责任点记录
                attendanceForPointAppService.insertAttendanceForPointByBeanApp(attendanceForPointApp);//新增考勤记录对应的代班责任点记录,2017-01-10
            }
        ...
        // 如果是环卫工
            ...
            //根据上下班时间段，责任区，日期以及请假人ID，获取请假人某个日期某个时间段的责任点集合
            //并将请假人的责任点集合添加到代班人某个时间段责任点集合中
            //并将代班责任点的is_coverwork置为1.
            AttendanceForPointApp attendanceForPointApp = new AttendanceForPointApp();
            attendanceForPointApp.setEmployeeId(coverId);
            attendanceForPointApp.setAreaId(dutyAreaId);
            attendanceForPointApp.setRecordDate(attendance.getRecordDate());
            ...
            attendanceForPointApp.setDutyOnTime(dutyOnTime);
            attendanceForPointApp.setIsCoverwork(1);
            attendanceForPointApp.setLeaveId(leaveId);
            attendanceForPointAppService.insertAttendanceForPointByLeaveApp(attendanceForPointApp);//将请假人某个时间段的责任点集合添加给代班人，2017-01-10
            attendanceForPointAppService.updateAttendanceForPointAppByBeanApp(attendanceForPointApp);//将代班责任点记录的is_coverwork设置为1.2016-01-10
    ...
    public AttendanceForPlansApp getAllCityWorkAttLastTimeForApp(Integer employeeId) {
        return attendanceForPlansDao.getInfoById("getAllCityWorkAttLastTimeForApp", employeeId);
    }
	...
	public int updateRegisterOrLogoutStatusApp(AttendanceForPlansApp attendanceForPlansApp) {
		return attendanceForPlansDao.updateInfoByBean("updateRegisterOrLogoutStatusApp", attendanceForPlansApp);
	}
	...
	public List<DutyRecodeApp> getLastBanciAttendanceForPlansByListAppForMOV(List<AttendanceForPlansApp> workPlansList) {
		return attendanceForPlansDao.getLastBanciAttendanceForPlansByListAppForMOV(workPlansList);
	}
}
