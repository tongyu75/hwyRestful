 ...
 */
@Controller
@RequestMapping(value = "/userAreaController")
...
//			String userKey = getUserKey();//模拟登陆，将用户信息存到缓存服务器中,在页面中测试时，将注释掉，且释放下面的代码
	...
	public String selectUserAreaList(String acceptContent,HttpServletResponse response, HttpServletRequest request){
		...
		try {
			...
			//3.查询员工归属责任区记录总条数，2016-11-10 : 只查询环卫工
			int total = userAreaService.getUserAreaCount(userArae);//查询员工归属责任区记录总条数，2016-09-29，2016-11-10 : 只查询环卫工
			
			//4.查询员工归属责任区记录集合，分页，2016-11-10 : 只查询环卫工
			List<UserArea> rows = userAreaService.getUserAreaList(userArae);//查询员工归属责任区记录集合，2016-09-29，分页，2016-11-10 : 只查询环卫工
	...
	public String selectAllUserAreaiList(String acceptContent, HttpServletResponse response, HttpServletRequest request){
		...
		//3.查询员工归属责任区记录总条数，2016-11-10 : 只查询环卫工
		int total = userAreaService.getUserAreaCount(userArea);//查询员工归属责任区记录总条数，2016-09-29，2016-11-10 : 只查询环卫工
		
		//4.查询员工归属责任区记录集合，不分页，2016-11-10 : 只查询环卫工
		List<UserArea> rows = userAreaService.getAllUserAreaListByBean(userArea);//查询员工归属责任区记录集合，2016-09-29，不分页，2016-11-10 : 只查询环卫工
