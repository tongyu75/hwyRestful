...
<body>
	<table id="dutyPlan_khy" class="easyui-datagrid"
		style="width: 100%; height: 100%"
		data-options="
				iconCls: 'icon-edit',
				singleSelect: true,
				toolbar: '#tb_khy',
				url: '<%=basePath %>mission/duty_getDutyList.action?roles_value=3',
				method: 'get',
					...
					data-options="field:'employeeId',width:80,align:'center'
								...
								,editor:{
									...
									options:{
										...
										idField: 'employeeId',
										textField: 'showname',
										url: '<%=basePath %>area/dictionary_getUsers4Combogrid.action?role_value=3',
										method: 'post',
										columns: [[
							...
							formatter:function(value,row){
								...
								,editor:{
									...
									options:{
										...
										url: '<%=basePath %>mission/banci_getBanciList.action?role_value=3',
										method: 'post',
										columns: [[
	...
	<div id="tb_khy" style="height: auto">
		<table cellpadding="5" style="color: #000000;font-size: 12px;display:none">
			<tr>
				<td>人员姓名</td>
				<td>
					<input id="dp_employeeId_khy" class="easyui-combogrid" data-options="panelWidth:220,mode:'remote',idField:'employeeId',textField:'showname'
				...
				<td>出勤起始日期</td>
				<td>
					<input id="dp_dutyFromDate_khy" class="easyui-datebox" />
				</td>
			</tr>
			<tr>
				<td>任务类型</td>
				<td>
					<input id="dp_dutyType_khy" class="easyui-combobox" data-options="url:'<%=basePath %>area/dictionary_getDutyWorkType.action',valueField:'workType',textField:'workName'" />
				</td>
				<td>&nbsp;</td>
				<td>
					<a href="#" class="easyui-linkbutton" iconCls="icon-search" id="dp_Search_khy">查询</a>
					&nbsp;
					<a href="#" class="easyui-linkbutton" iconCls="icon-cancel" id="dp_Clear_khy">清空</a>
				</td>
			</tr>
		</table>
		...
		<table cellpadding="5" style="color: #000000;font-size: 12px;">
			<tr>
				<td>责任区</td>
				<td><input class="easyui-combogrid" style="width:150px" id="dp_khy_areaId"
					data-options="panelWidth: 200,
								...
								fitColumns: true,
								onSelect:function(){
									var areaId=$('#dp_khy_areaId').combogrid('getValue');
									if(areaId!=''){
										$('#dp_khy_pointId').combobox('reload','<%=basePath %>area/dictionary_getDutyPoints4Combox.action?areaId='+areaId);
										$('#dp_khy_pointId').combobox('setValue','');
										$('#dp_khy_pointId').combobox('setText','');
									}
								}		
					">
				</td>
				<!-- <td>责任点</td>
				<td>
					<input id="dp_khy_pointId" class="easyui-combobox" data-options="valueField:'id',textField:'point_name'" />
				</td> -->
			</tr>
		</table>
	</div>
	<script type="text/javascript" src="<%=basePath%>dutyplan/dutyPlanManage.js"></script>
	<script type="text/javascript" src="<%=basePath%>dutyplan/dutyPlanManage_khy.js"></script>
</body>
</html>